...
func TestDuplicateRegister(t *testing.T) {
	...
	opts.SetConnectionLostHandler(func(client MQTT.Client, err error) {
		t.Log("ConnectionLost", err.Error())
	})
	opts.SetOnConnectHandler(func(client MQTT.Client) {
		t.Log("OnConnectHandler")
	})
	err := w.Connect(opts)
	if err != nil {
		t.Fatal(err)
	...
	resp, _ = w.Request("Wechat/HD_Wechat_RegisterMQTTPlugin", []byte(`{"name":"testPlugin","description":"测试模块"}`))
	if resp.Ret != common.RetCodeOK {
		t.Fatalf("注册plugin失败: %s", resp.Msg)
	}
	resp, _ = w.Request("Wechat/HD_Wechat_RegisterMQTTPlugin", []byte(`{"name":"testPlugin","description":"测试模块"}`))
	if resp.Ret != common.RetCodeBadRequest || resp.Msg != "duplicate registered" {
		t.Fatal("重复注册Plugin也能成功")
	}
	resp, _ = w.Request("Wechat/HD_Upload_RegisterMQTTUploader", []byte(`{"name":"testUploader","description":"测试上传者","uploadListenerTopic":"testFunc"}`))
	if resp.Ret != common.RetCodeOK {
		t.Fatalf("注册plugin失败: %s", resp.Msg)
	}
	resp, _ = w.Request("Wechat/HD_Upload_RegisterMQTTUploader", []byte(`{"name":"testUploader","description":"测试上传者","uploadListenerTopic":"testFunc"}`))
